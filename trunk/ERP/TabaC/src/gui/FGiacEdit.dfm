object fmGiacEdit: TfmGiacEdit
  Left = 259
  Top = 158
  ActiveControl = Panel1
  Caption = 'Mostra/modifica giacenze tabacchi'
  ClientHeight = 308
  ClientWidth = 461
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 461
    Height = 41
    Align = alTop
    TabOrder = 0
    object btCompleta: TSpeedButton
      Left = 233
      Top = 8
      Width = 25
      Height = 25
      Hint = 'Aggiungi mancanti'
      Glyph.Data = {
        
        00333377737FFFFF773333303300000003333337337777777333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btCompletaClick
    end
    object btPack: TSpeedButton
      Left = 258
      Top = 8
      Width = 25
      Height = 25
      Hint = 'Compatta e ricalcola totali'
      Glyph.Data = {
        
        9F9977F77F37F3377F77993CC330FFF99F997737733733377377}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btPackClick
    end
    object navGiac: TRGNavigator
      Left = 10
      Top = 8
      Width = 200
      Height = 25
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbPrint, nbRefresh, nbDelete, nbEdit]
      ConfirmDelete = False
      ConfirmEdit = False
      ConfirmPost = False
      ConfirmCancel = False
      TabOrder = 0
      OnClick = navGiacClick
      ColorScroll = ncBlack
      ColorFunc = ncBlack
      ColorCtrl = ncBlack
      ColorTool = ncBlack
      SizeOfKey = X4
      CaptionExtra1 = 'Extra1'
      CaptionExtra2 = 'Extra2'
      CaptionExtra3 = 'Extra3'
      DataSource = dsGiacLst
    end
  end
  object pnList: TPanel
    Left = 0
    Top = 41
    Width = 461
    Height = 100
    Align = alTop
    BevelInner = bvLowered
    BorderWidth = 4
    Caption = 'pnList'
    TabOrder = 1
    object DBGrid1: TDBGrid
      Left = 6
      Top = 6
      Width = 449
      Height = 88
      Align = alClient
      BorderStyle = bsNone
      DataSource = dsGiacLst
      Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clBlack
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'DATA'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATAPREZ'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'KGC'
          Title.Alignment = taCenter
          Width = 78
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VAL'
          Title.Alignment = taCenter
          Visible = True
        end>
    end
  end
  object pnMovs: TPanel
    Left = 0
    Top = 144
    Width = 461
    Height = 164
    Align = alClient
    BevelInner = bvLowered
    BorderWidth = 4
    Caption = 'pnMovs'
    TabOrder = 2
    object DBGrid2: TDBGrid
      Left = 6
      Top = 6
      Width = 449
      Height = 152
      Align = alClient
      BorderStyle = bsNone
      DataSource = dsGiacMov
      Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clBlack
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnEnter = DBGrid2Enter
      OnExit = DBGrid2Exit
      Columns = <
        item
          Expanded = False
          FieldName = 'CODI'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CodS'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Desc'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'GIAC'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'GiacKG'
          Title.Alignment = taCenter
          Visible = True
        end>
    end
  end
  object RxSplitter1: TRxSplitter
    Left = 0
    Top = 141
    Width = 461
    Height = 3
    ControlFirst = pnList
    ControlSecond = pnMovs
    Align = alTop
  end
  object dsGiacLst: TDataSource
    DataSet = tbGiacLst
    Left = 172
    Top = 200
  end
  object dsGiacMov: TDataSource
    DataSet = tbGiacMov
    Left = 226
    Top = 200
  end
  object tbGiacLst: TTable
    BeforeInsert = AbortOperation
    BeforeDelete = tbGiacLstBeforeDelete
    DatabaseName = 'DB'
    TableName = 'GIACLIST.DB'
    Left = 160
    Top = 210
    object tbGiacLstPGIA: TIntegerField
      DisplayWidth = 6
      FieldName = 'PGIA'
      Visible = False
    end
    object tbGiacLstDATA: TDateField
      Alignment = taRightJustify
      DisplayLabel = 'Data'
      FieldName = 'DATA'
      Required = True
    end
    object tbGiacLstDATAPREZ: TDateField
      Alignment = taRightJustify
      DisplayLabel = 'Data Prezzi'
      FieldName = 'DATAPREZ'
    end
    object tbGiacLstKGC: TFloatField
      DisplayLabel = 'Totale (KgC)'
      DisplayWidth = 9
      FieldName = 'KGC'
      DisplayFormat = '0.000'
    end
    object tbGiacLstVAL: TCurrencyField
      DisplayLabel = 'Totale (L.)'
      DisplayWidth = 15
      FieldName = 'VAL'
    end
  end
  object tbGiacMov: TTable
    BeforeInsert = AbortOperation
    BeforeDelete = tbGiacMovBeforeDelete
    OnCalcFields = tbGiacMovCalcFields
    DatabaseName = 'DB'
    IndexFieldNames = 'PGIA;CODI'
    MasterFields = 'PGIA'
    MasterSource = dsGiacLst
    TableName = 'GIACMOVS.DB'
    Left = 219
    Top = 210
    object tbGiacMovPGIA: TIntegerField
      FieldName = 'PGIA'
      Required = True
      Visible = False
    end
    object tbGiacMovCODI: TSmallintField
      DisplayLabel = 'Cod.'
      DisplayWidth = 5
      FieldName = 'CODI'
      Required = True
    end
    object tbGiacMovCodS: TStringField
      DisplayLabel = 'Cod. Sig.'
      FieldKind = fkCalculated
      FieldName = 'CodS'
      Size = 4
      Calculated = True
    end
    object tbGiacMovDesc: TStringField
      DisplayLabel = 'Nome tabacco'
      FieldKind = fkCalculated
      FieldName = 'Desc'
      Size = 30
      Calculated = True
    end
    object tbGiacMovGIAC: TIntegerField
      DisplayLabel = 'Giacenza'
      FieldName = 'GIAC'
      Required = True
    end
    object tbGiacMovGiacKG: TFloatField
      DisplayLabel = 'Giacenza (KgC)'
      FieldKind = fkCalculated
      FieldName = 'GiacKG'
      DisplayFormat = '0.00'
      Calculated = True
    end
  end
  object tbTaba: TTable
    DatabaseName = 'DB'
    TableName = 'TABACCHI.DB'
    Left = 95
    Top = 215
    object tbTabaCODI: TSmallintField
      FieldName = 'CODI'
      Required = True
    end
    object tbTabaCODS: TStringField
      FieldName = 'CODS'
      Required = True
      Size = 4
    end
    object tbTabaDESC: TStringField
      FieldName = 'DESC'
      Required = True
      Size = 30
    end
    object tbTabaATTV: TBooleanField
      FieldName = 'ATTV'
    end
    object tbTabaQTAC: TSmallintField
      FieldName = 'QTAC'
    end
  end
end

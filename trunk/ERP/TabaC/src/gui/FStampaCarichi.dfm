object fmStampaCarichi: TfmStampaCarichi
  Left = 241
  Top = 218
  BorderStyle = bsDialog
  Caption = 'Stampa tabacchi ordinati'
  ClientHeight = 148
  ClientWidth = 224
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  Scaled = False
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 16
    Top = 10
    Width = 49
    Height = 13
    Caption = 'Ordina per'
  end
  object Label1: TLabel
    Left = 16
    Top = 35
    Width = 50
    Height = 13
    Caption = 'Data Inizio'
  end
  object Label3: TLabel
    Left = 16
    Top = 60
    Width = 46
    Height = 13
    Caption = 'Data Fine'
  end
  object btCancel: TBitBtn
    Left = 126
    Top = 85
    Width = 89
    Height = 25
    Caption = 'Annulla'
    TabOrder = 2
    Kind = bkCancel
    Margin = 5
    Spacing = -1
  end
  object cbOrder: TComboBox
    Left = 75
    Top = 5
    Width = 110
    Height = 21
    Style = csDropDownList
    ItemHeight = 0
    TabOrder = 0
    OnChange = cbOrderChange
  end
  object btExport: TBitBtn
    Left = 126
    Top = 115
    Width = 89
    Height = 25
    Caption = '&Esporta'
    TabOrder = 4
    OnClick = PrintReport
    Glyph.Data = {
      
      3333777777333333333333333333333333333333333333333333}
    Margin = 5
    NumGlyphs = 2
    Spacing = -1
  end
  object btPrint: TBitBtn
    Left = 11
    Top = 85
    Width = 89
    Height = 25
    Caption = '&Stampa'
    TabOrder = 1
    OnClick = PrintReport
    Glyph.Data = {
      
      33333337FFFF7733333333300000033333333337777773333333}
    Margin = 5
    NumGlyphs = 2
    Spacing = -1
  end
  object btPreview: TBitBtn
    Left = 11
    Top = 115
    Width = 89
    Height = 25
    Caption = '&Anteprima'
    TabOrder = 3
    OnClick = PrintReport
    Glyph.Data = {
      
      5555555555555555555555555555555555555555555555555555}
    Margin = 5
    NumGlyphs = 2
    Spacing = -1
  end
  object iDataI: TDateEdit
    Left = 75
    Top = 30
    Width = 111
    Height = 21
    NumGlyphs = 2
    TabOrder = 5
  end
  object iDataF: TDateEdit
    Left = 75
    Top = 55
    Width = 111
    Height = 21
    NumGlyphs = 2
    TabOrder = 6
  end
  object Report: TeLineReport
    AutoCR = False
    PageHeight = 66
    PageWidth = 132
    HeaderSize = 3
    FooterSize = 0
    ReportName = 'Listino Tabacchi'
    DeviceKind = 'Preview'
    OnPageHeader = ReportPageHeader
    OnSetupDevice = ReportSetupDevice
    Left = 31
    Top = 100
  end
  object qryDettagli: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT DISTINCT'
      
        '  PreLst.DATA DataPrez, PreMov.PREZ Prez, SUM(CarMov.CARI) SumCa' +
        'r'
      ''
      'FROM'
      '  "Prezlist.db" PreLst,'
      '  "Prezmovs.db" PreMov,'
      '  "Carilist.db" CarLst,'
      '  "Carimovs.db" CarMov'
      ''
      'WHERE'
      
        '      (CarLst.DATA IS NOT NULL)and (CarLst.DATA >= :DataI) and (' +
        'CarLst.DATA <= :DataF)'
      '  AND (PreMov.CODI = :CodI)'
      '  AND (CarMov.CODI = :CodI)'
      '  AND (CarLst.DATAPREZ = PreLst.DATA)'
      '  AND (CarMov.PCAR = CarLst.PCAR)'
      '  AND (PreMov.PPRE = PreLst.PPRE)'
      ''
      'GROUP BY'
      '  PreLst.DATA, PreMov.PREZ'
      ''
      'ORDER BY'
      '  PreLst.DATA, PreMov.PREZ')
    Left = 65
    Top = 100
    ParamData = <
      item
        DataType = ftDate
        Name = 'DataI'
        ParamType = ptUnknown
        Value = 2d
      end
      item
        DataType = ftDate
        Name = 'DataF'
        ParamType = ptUnknown
        Value = 402133d
      end
      item
        DataType = ftInteger
        Name = 'CodI'
        ParamType = ptUnknown
        Value = 2
      end
      item
        DataType = ftInteger
        Name = 'CodI'
        ParamType = ptUnknown
        Value = 2
      end>
    object qryDettagliDataPrez: TDateField
      FieldName = 'DataPrez'
      Origin = '"Prezlist.DB".DATA'
    end
    object qryDettagliPrez: TCurrencyField
      FieldName = 'Prez'
      Origin = '"PREZMOVS.DB".PREZ'
    end
    object qryDettagliSumCar: TFloatField
      FieldName = 'SumCar'
      Origin = '"CARIMOVS.DB".CARI'
    end
  end
  object tbTaba: TTable
    DatabaseName = 'DB'
    TableName = 'TABACCHI.DB'
    Left = 91
    Top = 95
    object tbTabaCODI: TSmallintField
      FieldName = 'CODI'
      Required = True
    end
    object tbTabaCODS: TStringField
      FieldName = 'CODS'
      Required = True
      Size = 4
    end
    object tbTabaTIPO: TSmallintField
      FieldName = 'TIPO'
    end
    object tbTabaPROD: TSmallintField
      FieldName = 'PROD'
    end
    object tbTabaCRIT: TSmallintField
      FieldName = 'CRIT'
    end
    object tbTabaDESC: TStringField
      FieldName = 'DESC'
      Required = True
      Size = 30
    end
    object tbTabaATTV: TBooleanField
      FieldName = 'ATTV'
    end
    object tbTabaMULI: TSmallintField
      FieldName = 'MULI'
    end
    object tbTabaQTAS: TSmallintField
      FieldName = 'QTAS'
    end
    object tbTabaQTAC: TSmallintField
      FieldName = 'QTAC'
    end
    object tbTabaQTAM: TSmallintField
      FieldName = 'QTAM'
    end
    object tbTabaDIFR: TSmallintField
      FieldName = 'DIFR'
    end
  end
end

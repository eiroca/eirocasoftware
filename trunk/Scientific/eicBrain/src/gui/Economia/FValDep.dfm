object fmValDep: TfmValDep
  Left = 0
  Top = 0
  Caption = 'Valore futuro deposito'
  ClientHeight = 151
  ClientWidth = 313
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poOwnerFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 42
    Top = 127
    Width = 74
    Height = 13
    Caption = 'Valore futuro ='
  end
  object lOutput: TLabel
    Left = 122
    Top = 127
    Width = 6
    Height = 13
    Caption = '?'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 185
    Top = 89
    Width = 11
    Height = 13
    Caption = '%'
  end
  object iPeriodi: TLabeledEdit
    Left = 122
    Top = 62
    Width = 57
    Height = 21
    EditLabel.Width = 83
    EditLabel.Height = 13
    EditLabel.Caption = 'Numero di periodi'
    LabelPosition = lpLeft
    TabOrder = 0
    Text = '10'
  end
  object iPV: TLabeledEdit
    Left = 122
    Top = 8
    Width = 57
    Height = 21
    EditLabel.Width = 97
    EditLabel.Height = 13
    EditLabel.Caption = 'Investimento iniziale'
    LabelPosition = lpLeft
    TabOrder = 1
    Text = '100'
  end
  object btCancel: TBitBtn
    Left = 206
    Top = 39
    Width = 75
    Height = 25
    TabOrder = 5
    Kind = bkCancel
  end
  object btCalc: TBitBtn
    Left = 206
    Top = 8
    Width = 75
    Height = 25
    Caption = 'Calcola'
    Default = True
    TabOrder = 4
    OnClick = btCalcClick
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object iTasso: TLabeledEdit
    Left = 122
    Top = 89
    Width = 57
    Height = 21
    EditLabel.Width = 86
    EditLabel.Height = 13
    EditLabel.Caption = 'Tasso di interesse'
    LabelPosition = lpLeft
    TabOrder = 3
    Text = '5'
  end
  object iPMT: TLabeledEdit
    Left = 122
    Top = 35
    Width = 57
    Height = 21
    EditLabel.Width = 77
    EditLabel.Height = 13
    EditLabel.Caption = 'Depositi regolari'
    LabelPosition = lpLeft
    TabOrder = 2
    Text = '100'
  end
end

...
class syntax_plugin_divalign2_right extends DokuWiki_Syntax_Plugin {
    ...
    function getInfo(){
        return array(
            ...
            'name'   => 'divalign2',
            'desc'   => 'Set right alignment with ;;# content ;;#',
    ...
    function render($mode, &$renderer, $data) {
        ...
        if ($mode == 'odt' ){
          ...
          $st = '
          <style:style style:name="Text.Divalign.Right" style:display-name="Text.Divalign.Right" style:family="paragraph" style:parent-style-name="Text_20_body">
               <style:paragraph-properties fo:text-align="right" style:justify-single-word="false"/>
          </style:style>';
          $renderer->autostyles['Text.Divalign.Right']= $st;
          $center_defined= true;
          switch ($state) {
            case DOKU_LEXER_ENTER: {
              $renderer->doc.= '<text:p ';
              $renderer->doc.= 'text:style-name="Text.Divalign.Right">';
              break;
              }
            case DOKU_LEXER_EXIT: {
              $renderer->doc.= '</text:p>';
